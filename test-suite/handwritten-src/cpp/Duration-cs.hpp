...
struct Duration<Rep, DurationPeriod<Ratio>> {
    ...
    static CppType ToCpp(CsType dt) {
        return std::chrono::duration_cast<CppType>(std::chrono::duration<long long, std::ratio<1, 10000000>>{dt.Ticks});
    }
    static CsType FromCpp(CppType dt) {
        auto ms = std::chrono::duration_cast<std::chrono::duration<long long, std::ratio<1, 10000000>>>(dt).count();
